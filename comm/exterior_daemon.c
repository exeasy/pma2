...
int exterior_daemon_init(void)
{
	...
	add_ctl(upload_handle, OPS_PMS_POLICY_SEND,       handle_ops_pms_policy_send,		1);
	add_ctl(upload_handle, OPS_PMS_MARI_SEND,         handle_ops_pms_mari_send,			1);
	add_ctl(upload_handle, NEIGHBOR_LIST,             handle_ops_pms_neighbor_list,		1);
	add_ctl(upload_handle, TC_TO_PMA_INTERFACE_INFO,  handle_trans_message,				1);
	add_ctl(upload_handle, TC_TO_PMA_FLOW_INFO,       handle_trans_message,				1);
//canceled	add_ctl(upload_handle, OSPF_SPF_SIGNAL,           handle_ospf_spf_signal_to_dbm,	1);
